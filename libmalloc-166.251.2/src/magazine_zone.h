 ...
 */
// Tanner: 每2个bit位表示一个block(16字节)信息
#define CEIL_NUM_TINY_BLOCKS_WORDS (((NUM_TINY_BLOCKS + 31) & ~31) >> 5)  // 2017 能表示 （(2017 * 64）/ 2）个block
 ...
 */
// Tanner: 去掉 (NUM_TINY_BLOCKS * TINY_QUANTUM) 能占用的bit位, 后20个bit位，得到region基地址 => TINY_BLOCKS_ALIGN: 20
...
typedef struct tiny_region {
	// Tanner: Data，malloc申请的空间在这里面
	tiny_block_t blocks[NUM_TINY_BLOCKS];   // sizeof(tiny_block_t) => 16;  NUM_TINY_BLOCKS => 64520    252 pages + 128 bytes
	...
	tiny_header_inuse_pair_t pairs[CEIL_NUM_TINY_BLOCKS_WORDS];  // CEIL_NUM_TINY_BLOCKS_WORDS => 2017  (2017 * 4 * 8) = 64544 bits for header
 ...
 */
// Tanner: (NUM_TINY_CEIL_BLOCKS - 1) => 111...111{16 bit};  SHIFT_TINY_QUANTUM: 4
// 后20bit位表示region基地址到ptr的距离大小(blocks最多占用20bit位)， 除以16(block的大小)，得到当前ptr的index     num >> 4 (除以16)
...
typedef struct small_region {
	small_block_t blocks[NUM_SMALL_BLOCKS];  // small_block_t => 512 bytes
	region_trailer_t trailer;
	msize_t small_meta_words[NUM_SMALL_BLOCKS];  // msize_t => 4 bytes

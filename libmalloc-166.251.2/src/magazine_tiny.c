...
// Tanner: 从rack的magazines list偏移为0(DEPOT_MAGAZINE_INDEX=-1, rack->magazines初始化时，偏移+1)的magazine中取出一个有msize大小region，然后从取出的magazine移除该region节点，然后添加到tiny_mag_ptr(region_trailer_t结构的双向链表)节点中
static boolean_t
tiny_get_region_from_depot(rack_t *rack, magazine_t *tiny_mag_ptr, mag_index_t mag_index, msize_t msize)
{
	...
	// Tanner: 获取该region的使用信息，并将该region中的free_memory_block从depot_ptr的空闲链表中移除
	...
	// Tanner: 将该region中的free_memory_block添加到tiny_mag_ptr的空闲链表中
...
tiny_free_no_lock(rack_t *rack, magazine_t *tiny_mag_ptr, mag_index_t mag_index, region_t region, void *ptr, msize_t msize)
{
	...
	// We try to coalesce(合并) this block with the preceeding one
	// Tanner: 向左合并
								...
								void *aligned_address)
{
	...
	int offset_msize = 0;
#endif
	ptr = (void *)((uintptr_t)aligned_address + TINY_BYTES_FOR_MSIZE(offset_msize));   // 随机起始位置
	set_tiny_meta_header_in_use(ptr, msize);
	tiny_mag_ptr->mag_num_objects++;
...
// Tanner: tiny_malloc_from_free_list
// 0. 从magazine的空闲链表数组(16, 32, 64大小递增...)取出一个元素
// 1. 从空闲链表数组取出msize(1, 2, 3...)对应的空闲链表，如果有元素，则返回该元素地址；没有，goto 2
// 2. 根据magazine的bitmap(表示空闲链表是否有元素可用)信息，找到第一个大于msize有元素的空闲链表，如果没有可用的空闲链表，goto 3; 如果该空闲链表有元素，goto 5, return 该元素地址; 没有，goto 4;
// 3. 从magazine最后申请的region中申请内存大小(该region是mmap新鲜出炉的，有些内存重来没被使用过(free)，顾不在free_list中)，该region从当前已使用的内存地址开始按照msize分配内存块，并设置刚刚分配的blocks对应region的Meta_Data的bit信息，return 该地址
// 4. 从空闲链表数组最后一个元素，取出的空闲链表元素为1008字节大小，如果该空闲链表有元素，goto 4, return 该元素地址; 没有, return null;
// 5. 2和3取出来的元素大小是大于msize的，因此将多余的大小空间分割出来并添加到对应大小的空闲链表中
// end: 从空闲链表取出元素后，判断下一个元素是否存在? 存在，修改元素指向，不存在，设置magazine对应bitmap对应链表信息;
void *
tiny_malloc_from_free_list(rack_t *rack, magazine_t *tiny_mag_ptr, mag_index_t mag_index, msize_t msize)
{
	tiny_free_list_t *ptr;
	msize_t this_msize;
	// Tanner: slot: 根据msize计算的空闲链表数组的index
	grain_t slot = tiny_slot_from_msize(msize);
	free_list_t *free_list = tiny_mag_ptr->mag_free_list;
	free_list_t *the_slot = free_list + slot;
	...
	// Tanner: __builtin_ctzl => 返回从第1位开始 bit为0的个数 (bitmap一共64位)
	slot = BITMAPV_CTZ(bitmap);
	limit = free_list + NUM_TINY_SLOTS;
	free_list += slot;
...
// Tanner: tiny_malloc
// 0. tiny_magazine; 根据当前线程取出rack对应的magazine (为了并发分配内存的性能，运行在同一核心的多线程会操作同一批内存区域 magazine->regions)
// 1. cache; 上一次free的内存块地址和块大小会缓存起来，和这次malloc大小比较, 如过命中，直接返回该地址，否则，goto 2
// 2. free_lists; 从magazine的空闲链表数组中寻找内存块，如果找到，返回地址; 没找到, goto 3
// 3. first_magazine; 从rack找到第一个magazine，然后从该magazine中找到一块空余空间大于msize的region，没找到，goto 4; 找到的话 遍历该region中空闲的内存块添加到tiny_magazine的free_lists中; 再从free_lists中寻找free内存块(即第2步)，没找到，goto 4, 找到的话，return 该内存块地址
// 4. mmap; 向内核申请一块region内存
void *
tiny_malloc_should_clear(rack_t *rack, msize_t msize, boolean_t cleared_requested)
{
...
#if DEBUG_MALLOC
	if (DEPOT_MAGAZIfNE_INDEX == mag_index) {
	...
	while (1) {
		...
		if (!tiny_mag_ptr->alloc_underway) {  // 不加锁；在同一核中操作该变量，变量为YES时，让出时间片；变量为NO时，进行写操作(该变量为volatile修饰，每次读取从内存重新加载到寄存器，而不会利用CPU缓存) 保证了读的虚拟并发，写的单一原则
...
// Tanner: tiny_free
// 0. 根据释放的地址，计算出改地址meta信息；包括是否释放过以及申请的大小(根据地址可以计算出region的起始地址，再根据region的meta_data信息计算该地址信息), (is_free for Double Free)
// 1. 将该地址和内存块大小缓存到magazine的属性中，并将之前的缓存添加到对应的free_list中  (cache for UAF)
void
...
{
	msize_t msize = TINY_MSIZE_FOR_BYTES(size + TINY_QUANTUM - 1);    // (size + 15) >> 4
	unsigned found = 0;

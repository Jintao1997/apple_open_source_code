...
#define NANO_MAG_SIZE 		(1 << NANO_MAG_BITS)  // 64
...
typedef struct chained_block_s {
    uintptr_t			double_free_guard;		// 释放后设置的金丝雀值，防止double free; malloc后设置该值为0
...
typedef struct nano_meta_s {
    ...
    volatile uintptr_t		slot_current_base_addr;  // 当前分配的内存区域起始地址
    volatile uintptr_t		slot_limit_addr;       // 当前区域能使用最大的地址 > slot_bump_addr + slot_bytes, 继续分配；否则向内核申请2M内存，slot_limit_addr += 128KB (当前2M内存区域的128KB用完，继续分配2M虚拟内存)
    volatile size_t		slot_objects_mapped;       // 每次2M中的前128KB内存用完时，+=(128KB / slot_bytes)
    volatile size_t		slot_objects_skipped;      // 第一次分配随机跳过多少个对象(对象大小为slot_bytes)
    bitarray_t			slot_madvised_pages;
    // position on cache line distinct from that of slot_LIFO
    volatile uintptr_t		slot_bump_addr MALLOC_NANO_CACHE_ALIGN;  // 当前分配的地址(递增,每次 += slot_bytes)
    volatile boolean_t		slot_exhausted; // 能否分配空间
    unsigned int		slot_bytes;   // 16, 32, 48, 64 ... 256(唯一，固定不变)
    unsigned int		slot_objects; //  (128KB / slot_bytes) (唯一，固定不变)
} *nano_meta_admin_t;
...
typedef struct nanozone_s {
    ...
    struct nano_meta_s		meta_data[NANO_MAG_SIZE][NANO_SLOT_SIZE];   // NANO_MAG_SIZE => 64; NANO_SLOT_SIZE => 16
